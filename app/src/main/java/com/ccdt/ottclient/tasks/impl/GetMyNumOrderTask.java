 ...
 * 获取设备订购的按次消费资产信息
 */
public class GetMyNumOrderTask extends BaseTask<Map<String, String>> {

    public GetMyNumOrderTask(TaskCallback callback) {
        super(callback);
    }
    ...
    protected TaskResult doInBackground(Map<String, String>... params) {
        ...
        ret.taskId = Constants.TASK_GETMYNUMORDER;
        ...
        if (params != null && params.length > 0) {
            ...
            if (map.containsKey("terminalType")) {
                terminalType = map.get("terminalType");
            }
            if (map.containsKey("posterType")) {
                posterType = map.get("posterType");
            }
        }

        try {
            ret.data =  MyApp.getOTTApi().getMyNumOrder(state, pageSize, pageNumber, terminalType, posterType);
            ret.code = 200;
            ret.message = "success";
        } catch (Exception e) {
